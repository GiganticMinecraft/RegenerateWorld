...
case object Multiverse {
  private val instance = INSTANCE.getServer.getPluginManager.getPlugin("Multiverse-Core").asInstanceOf[MultiverseCore]

  def fromBukkitWorld(world: World): Option[MultiverseWorld] = Option(instance.getMVWorldManager.getMVWorld(world))

  def regenWorld(world: MultiverseWorld, isNewSeed: Boolean, isRandomSeed: Boolean, newSeed: Option[String]): Boolean
