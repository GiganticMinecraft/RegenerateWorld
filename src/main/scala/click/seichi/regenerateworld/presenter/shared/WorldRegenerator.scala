...
case object WorldRegenerator {
  def regen(world: MultiverseWorld, seedPattern: SeedPattern, newSeed: Option[String]): Either[WorldRegenerationException, Unit] = {
    if (seedPattern == SeedPattern.NewSeed && newSeed.isEmpty) return Left(WorldRegenerationException.SeedIsRequired)
  ...
  def regenFromWorld(bukkitWorld: World, seedPattern: SeedPattern, newSeed: Option[String]): Either[WorldRegenerationException, Unit] = {
    for {
      mvWorld <- bukkitWorld.asMVWorld().toRight(WorldRegenerationException.BukkitWorldIsNotMVWorld)
