...
case object WorldRegenerator {
  def regen(world: MultiverseWorld, seedPattern: SeedPattern, newSeed: Option[String]): Either[WorldRegeneratorError, ()] = {
  ...
  def regenFromWorld(world: World, seedPattern: SeedPattern, newSeed: Option[String]): Either[WorldRegeneratorError, ()] = {
    for {
