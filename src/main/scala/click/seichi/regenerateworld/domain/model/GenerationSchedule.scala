...
case class GenerationSchedule(id: UUID, nextDateTime: ZonedDateTime, interval: Interval, seedPattern: SeedPattern, worlds: Set[String]) {
  def finish(now: ZonedDateTime): GenerationSchedule = {
    ...
    val newNextDateTime = if (maybeNextDateTime.isBefore(now)) now.plus(interval.value, interval.unit.chronoUnit) else maybeNextDateTime
...
object GenerationSchedule {
  def fromRepository(id: UUID, nextDateTime: String, intervalUnit: String, intervalValue: Long, seedPattern: String, worlds: Set[String]): Option[GenerationSchedule] = {
    for {
